...
def register(request):
    if request.method == 'POST':
        ...
        if not (8 <= len(password)) or not any(char.isupper() for char in password) or not any(char.isdigit() for char in password):
            return JsonResponse({'message': 'Username must be at least 8 characters and contain at least one uppercase letter and one digit', 'errorType': 3}, status=409)

        if not User.objects.filter(username=username).exists():
            if not User.objects.filter(email=email).exists():
                user = User.objects.create_user(username=username, email=email, password=make_password(password), first_name=first_name, last_name=last_name)
